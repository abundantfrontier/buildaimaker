...
/// Thin L1 helper stub for `Contents/Helpers/bam-voice-worker`.
///
/// Flow (spike):
/// 1. Resolve `Workers/python` pins root
/// 2. Verify L2 runtime-pins (lockfile + entry hashes, including voice_worker)
/// 3. Optionally check interpreter under managed env
/// 4. Print JSON `hello` and exit 0 (no real F5-TTS / multi-GB download in CI)
///
